 ...
 * - villareyonu.com (RSC id + api.villareyonu.com PriceList/Availability)
...
/** `46,200.00` (US), `46.200,00` (TR), `14.000` / `14,000` veya `46857,142857` → number */
function parseLocalizedMoney(raw: string): number {
  ...
  // Binlik nokta, ondalık yok: 14.000 / 46.200
  if (/^\d{1,3}(?:\.\d{3})+$/.test(s)) {
    return Number(s.replace(/\./g, ""));
  }
  // Binlik virgül, ondalık yok: 14,000 / 20,000
  if (/^\d{1,3}(?:,\d{3})+$/.test(s)) {
    return Number(s.replace(/,/g, ""));
  }
  // Ondalık virgül: 46.200,00 veya 46857,1428571429
...
function looksLikeVillajoye(pageUrl: string, html: string): boolean {
  try {
    const host = normalizeHost(new URL(pageUrl).hostname);
    if (!host.includes("villajoye")) return false;
    return (
      html.includes("_Api_avalbooked.php") ||
      html.includes("form_listing_id") ||
      html.includes("fiyat_hesaplama.php")
    );
  } catch {
    return false;
  }
...
function extractVillajoyePropertyId(html: string): string | null {
  const fromApi = html.match(/_Api_avalbooked\.php\?pro_id=\s*(\d+)/i)?.[1];
  if (fromApi) return fromApi;
  const fromForm = html.match(/form_listing_id["']\s*:\s*(\d+)/i)?.[1];
...
export function parseVillajoyeHtmlPeriods(
  ...
  const deposit = damageDeposit ?? extractDamageDeposit(html);
  const prepaymentRate = parsePercentRate(
    html.match(/Ön Ödeme Tutarı:\s*%?\s*(\d+)/i)?.[1] ?? ""
  );
  const periods: MappedVillaPricePeriod[] = [];
  const blockRe =
...
/** villajoye _Api_avalbooked bookings — giriş dahil, çıkış hariç. */
export function parseVillajoyeBookingOccupancy(
  bookings: { checkIn?: string; checkOut?: string }[]
): Map<string, VillaDayOccupancy> {
  const map = new Map<string, VillaDayOccupancy>();

  for (const booking of bookings) {
    const startKey = normalizeLooseDateKey(String(booking.checkIn ?? ""));
    const endKey = normalizeLooseDateKey(String(booking.checkOut ?? ""));
    if (!startKey || !endKey) continue;

    let cursor = parseDateKey(startKey);
    const end = parseDateKey(endKey);
    while (compareDates(cursor, end) < 0) {
      map.set(toDateKey(cursor), "BOOKED");
      cursor.setDate(cursor.getDate() + 1);
    }
  }
...
async function scrapeVillajoyeFromPage(
  ...
  if (!looksLikeVillajoye(pageUrl, html)) return null;

  const deposit = extractDamageDeposit(html);
  let periods = parseVillajoyeHtmlPeriods(html, deposit);
  const propertyId = extractVillajoyePropertyId(html);
  const occupancyByDateKey = new Map<string, VillaDayOccupancy>();
  ...
  if (propertyId) {
    ...
    try {
      const data = (await fetchJson(
        `${originFromUrl(pageUrl)}/admin/include/_Api_avalbooked.php?pro_id=${encodeURIComponent(propertyId)}`,
        { referer: pageUrl }
      )) as unknown;

      if (Array.isArray(data)) {
        const bookings = Array.isArray(data[1])
          ? (data[1] as { checkIn?: string; checkOut?: string }[])
          : [];
        for (const [key, value] of parseVillajoyeBookingOccupancy(bookings)) {
          occupancyByDateKey.set(key, value);
        }

        if (periods.length === 0 && Array.isArray(data[0])) {
          const dateKeys: string[] = [];
          const prices: number[] = [];
          for (const item of data[0] as { date?: string; text?: string }[]) {
            const key = normalizeLooseDateKey(String(item?.date ?? ""));
            const price = Math.round(
              parseLocalizedMoney(
                stripTags(String(item?.text ?? "")).replace(/[^\d.,]/g, "")
              )
            );
          ...
          dateKeys.sort();
          periods = collapseDailyPricesToPeriods(dateKeys, prices, "TL").map(
            (period) => ({
              ...period,
              damageDeposit: deposit.amount,
              damageDepositCurrency: deposit.currency,
        ...
        warnings.push("villajoye takvim API beklenen formatta değil");
      }
    } catch (error) {
      warnings.push(
        error instanceof Error
          ? `villajoye müsaitlik API başarısız: ${error.message}`
          : "villajoye müsaitlik API başarısız"
      );
    }
  } else {
    warnings.push("villajoye ilan id (pro_id) bulunamadı");
  }
  ...
  if (occupancyByDateKey.size === 0) {
    warnings.push(
      "villajoye fiyatları alındı; müsaitlik takvimi okunamadı veya boş"
    );
  }
...
const VILLAREYONU_API = "https://api.villareyonu.com";
...
function extractEscapedRscField(chunk: string, key: string): string | null {
...
/**
 * App Router RSC (`self.__next_f`) gömülü villa kartı.
 * villareyonu.com PriceList/Availability `id` (ör. 4390) kullanır; RoutingId (1036) değil.
 */
export function extractRscVillaApiEntity(
  html: string
): ReturnType<typeof extractVillaApiEntity> {
  const marker = html.search(/evkodu\\":\\"VR-/i);
  if (marker < 0) return null;

  const chunk = html.slice(Math.max(0, marker - 25000), marker + 6000);
  const routingId = extractEscapedRscField(chunk, "RoutingId");
  const ids = [...chunk.matchAll(/"id\\":\\"(\d+)\\"/g)].map((match) => match[1]!);
  const entityId =
    ids.find((id) => id !== routingId) ??
    extractEscapedRscField(chunk, "id") ??
    routingId;
  if (!entityId || !/^\d+$/.test(entityId)) return null;

  const hasarRaw = extractEscapedRscField(chunk, "hasar");
  const damageDeposit = hasarRaw
    ? Number(String(hasarRaw).replace(/[^\d.]/g, ""))
    : NaN;
  const depozito = extractEscapedRscField(chunk, "depozito");
  const symbol = extractEscapedRscField(chunk, "Symbol") ?? "₺";
  const currencyCode =
    extractEscapedRscField(chunk, "CurrencyCode") ??
    extractEscapedRscField(chunk, "FromCurrencyCode") ??
    "TRY";

  return {
    entityId,
    title:
      extractEscapedRscField(chunk, "baslik") ??
      extractEscapedRscField(chunk, "kisa_icerik"),
    symbol,
    currencyCode,
    damageDeposit:
...
export async function scrapeVillavillamFromPage(
  ...
  if (!entity && html.trim() && site.hostKey === "villareyonu") {
    entity = extractRscVillaApiEntity(html);
  }
  if (
    !entity &&
    html.trim() &&
 ...
 */
function looksLikeBirvillas(pageUrl: string): boolean {
  try {
    return normalizeHost(new URL(pageUrl).hostname).includes("birvillas");
  } catch {
    return false;
  }
}

function extractBirvillasListingId(pageUrl: string): string | null {
  try {
    const parts = new URL(pageUrl).pathname.split("/").filter(Boolean);
    const idx = parts.findIndex(
      (part) => part === "villa" || part === "listing"
    );
    if (idx < 0) return null;
...
type BirvillasDynamicPeriod = {
  startDate?: string;
  endDate?: string;
  price?: number;
  minimumStay?: number;
  minimumStayForCleaning?: number;
  name?: string;
};

type BirvillasListing = {
  id?: string;
  ...
  dynamicPricing?: BirvillasDynamicPeriod[];
  disabledDates?: string[];
  notSelectableDates?: string[];
};

function extractJsonObjectContaining(
  text: string,
  needle: string,
  ...
  while (true) {
    ...
    let esc = false;
    let end = -1;
    for (let j = start; j < Math.min(text.length, start + 500_000); j++) {
      const ch = text[j]!;
      if (inStr) {
        if (esc) esc = false;
        else if (ch === "\\") esc = true;
    ...
    if (!requiredSubstrings.every((part) => candidate.includes(part))) continue;
    return candidate;
  }
}

function parseBirvillasListingFromHtml(
  html: string,
  listingId: string
): BirvillasListing | null {
  const decoded = decodeNextFlightPayloads(html);
  const raw =
    extractJsonObjectContaining(decoded, `"id":"${listingId}"`, [
      '"dynamicPricing"',
      '"fees"',
    ]) ??
    extractJsonObjectContaining(html, `"id":"${listingId}"`, [
      '"dynamicPricing"',
      '"fees"',
    ]);
  if (!raw) return null;
  ...
  const normalized = raw
    ...
    .replace(/,\s*([}\]])/g, "$1");
  try {
    return JSON.parse(normalized) as BirvillasListing;
  } catch {
    return null;
  }
}

function birvillasCurrency(
  value: string | null | undefined
): VillaPeriodCurrency {
  const upper = (value ?? "TRY").toUpperCase();
...
function birvillasTitle(listing: BirvillasListing): string | null {
  const title = listing.title ?? listing.name;
  if (!title) return null;
  if (typeof title === "string") return title;
  return title.tr ?? title.en ?? null;
}

export function scrapeBirvillasFromHtml(
  pageUrl: string,
  html: string,
  warnings: string[]
): ScrapedVillaPage | null {
  if (!looksLikeBirvillas(pageUrl)) return null;
  if (!html.includes("dynamicPricing") && !html.includes("self.__next_f")) {
    return null;
  }

  const listingId = extractBirvillasListingId(pageUrl);
  if (!listingId) {
    warnings.push("birvillas ilan id URL'den okunamadı");
    return null;
  }

  const listing = parseBirvillasListingFromHtml(html, listingId);
  if (!listing) {
    warnings.push("birvillas RSC listing verisi okunamadı");
    return null;
  }

  const currency = birvillasCurrency(listing.currency);
  const cleaningFee = positiveInt(listing.fees?.cleaningFee);
  const damageDeposit = positiveInt(listing.fees?.damageDeposit);
  const defaultMinStay = positiveInt(listing.minDays);

  const periods: MappedVillaPricePeriod[] = [];
  for (const item of listing.dynamicPricing ?? []) {
    const startKey = normalizeLooseDateKey(String(item.startDate ?? ""));
    const endKey = normalizeLooseDateKey(String(item.endDate ?? ""));
    const price = positiveInt(item.price);
    if (!startKey || !endKey || !price) continue;
    const startDate = parseDateKey(startKey);
    const endDate = parseDateKey(endKey);
    if (compareDates(startDate, endDate) > 0) continue;
    ...
    periods.push(
      buildMappedPeriod({
        ...
        nightlyPrice: price,
        currency,
        minStayNights: positiveInt(item.minimumStay) ?? defaultMinStay,
        cleaningDayCount: positiveInt(item.minimumStayForCleaning),
        cleaningFee,
        cleaningFeeCurrency: currency,
        damageDeposit,
        damageDepositCurrency: currency,
  ...
  if (periods.length === 0) {
    warnings.push("birvillas dynamicPricing dönemleri boş");
    return null;
  }

  const occupancyByDateKey = new Map<string, VillaDayOccupancy>();
  for (const raw of listing.disabledDates ?? []) {
    const key = normalizeLooseDateKey(String(raw));
    if (key) occupancyByDateKey.set(key, "BOOKED");
  }
  ...
  if (occupancyByDateKey.size === 0) {
    warnings.push(
      "birvillas fiyatları alındı; disabledDates boş veya okunamadı"
    );
  }
  ...
  return {
    sourceHost: normalizeHost(new URL(pageUrl).hostname),
    strategy: "birvillas",
    pageTitle: birvillasTitle(listing) ?? extractPageTitle(html),
    periods,
    occupancyByDateKey,
    warnings,
  };
...
function mapPlatoCurrency(raw: string | null | undefined): VillaPeriodCurrency {
  const value = String(raw ?? "").toUpperCase();
  if (value.includes("POUND") || value.includes("GBP") || value.includes("£")) {
    return "GBP";
  }
  if (value.includes("EURO") || value.includes("EUR") || value.includes("€")) {
    return "EUR";
  }
  if (value.includes("USD") || value.includes("DOLLAR") || value.includes("$")) {
    return "USD";
  }
  return "TL";
}

function parsePlatoMoney(raw: unknown): number | null {
...
function extractPlatoVillaPayload(html: string): {
  name?: string;
  prices?: PlatoPriceRow[];
  dates?: PlatoDateRow[];
  details?: { damageDeposit?: number | string | null };
} | null {
  const match = html.match(
  ...
  try {
    ...
    if (
      !Array.isArray(villa.prices) &&
      !Array.isArray(villa.dates) &&
...
function platoDateKeyFromIso(raw: string | undefined): string | null {
  if (!raw) return null;
...
export function scrapePlatoMacrovillaFromHtml(
  pageUrl: string,
  html: string,
  warnings: string[]
): ScrapedVillaPage | null {
  const villa = extractPlatoVillaPayload(html);
  if (!villa) return null;
  const host = normalizeHost(new URL(pageUrl).hostname);
  const looksPlato =
    host.includes("villadenizi") ||
    /plato-static\.s3|macrovilla\.com/i.test(html);
  if (!looksPlato) return null;

  const depositRaw = Number(villa.details?.damageDeposit);
  const damageDeposit =
    Number.isFinite(depositRaw) && depositRaw > 0 ? Math.round(depositRaw) : null;
  ...
  const seen = new Set<string>();
  let sourceId = 1;
  for (const row of villa.prices ?? []) {
    const startKey = platoDateKeyFromIso(row.startedAt);
    const endKey = platoDateKeyFromIso(row.finishedAt);
    if (!startKey || !endKey) continue;
    const startDate = parseDateKey(startKey);
    const endDate = parseDateKey(endKey);
    if (!startDate || !endDate || compareDates(startDate, endDate) > 0) continue;
    const nightlyPrice = parsePlatoMoney(row.price);
    if (nightlyPrice == null) continue;
    const currency = mapPlatoCurrency(row.priceCurrency);
    const minStayNights = Number(row.minDay);
    const cleaningFee = parsePlatoMoney(row.cleaningPrice);
    const cleaningDayCount = Number(row.cleaningDay);
    const key = `${startKey}_${endKey}_${nightlyPrice}_${currency}`;
    if (seen.has(key)) continue;
    seen.add(key);
    periods.push(
      buildMappedPeriod({
        ...
        currency,
        minStayNights:
          Number.isFinite(minStayNights) && minStayNights > 0
            ? minStayNights
            : null,
        cleaningFee,
        cleaningDayCount:
          Number.isFinite(cleaningDayCount) && cleaningDayCount > 0
            ? cleaningDayCount
            : null,
        damageDeposit,
        damageDepositCurrency: currency,
  ...
  periods.sort((a, b) => compareDates(a.startDate, b.startDate));

  const occupancyByDateKey = new Map<string, VillaDayOccupancy>();
  for (const row of villa.dates ?? []) {
    ...
    const isOption = status.includes("OPTION") || status.includes("HOLD");
    if (!isBooked && !isOption) continue;
    const startKey = platoDateKeyFromIso(row.startedAt);
    const endKey = platoDateKeyFromIso(row.finishedAt);
    if (!startKey || !endKey) continue;
    ...
    for (const dateKey of keys) {
      ...
      occupancyByDateKey.set(dateKey, isOption ? "OPTION" : "BOOKED");
    }
  }

  if (periods.length === 0) {
    warnings.push(
      "Plato/Macrovilla: fiyat periyodu boş (sitede gizlenmiş veya tanımlanmamış olabilir)"
    );
  }
  if ((villa.dates?.length ?? 0) === 0) {
    warnings.push("Plato/Macrovilla: takvim (dates) boş");
  }

  return {
    sourceHost: host,
    strategy: "plato_macrovilla",
    pageTitle: villa.name ?? extractPageTitle(html),
    periods,
    occupancyByDateKey,
    warnings,
  };
...
function looksLikeEgetatilevleri(pageUrl: string, html: string): boolean {
  try {
  ...
  return (
    ...
    (html.includes('id="booking-availability-ranges"') ||
      html.includes('id="booking-blocked-ranges"'))
  );
}

function mapEgetatilevleriCurrency(raw: string | null | undefined): VillaPeriodCurrency {
  const value = String(raw ?? "").toUpperCase();
...
type EgetatilevleriSeasonalPricing = {
  StartDate?: string;
  EndDate?: string;
  PricePerNight?: number | string;
  WeeklyPrice?: number | string;
  MinStayDays?: number | string;
  MinStayDaysForCleaning?: number | string;
  CleaningPrice?: number | string;
  HeatingPrice?: number | string;
};

type EgetatilevleriBlockedRange = {
  start?: string;
  end?: string;
  status?: string;
  reason?: string;
...
export function parseEgetatilevleriPeriods(
  html: string
): MappedVillaPricePeriod[] {
  const raw = extractJsonScriptById(html, "booking-seasonal-pricings");
  if (!Array.isArray(raw)) return [];

  const base = extractJsonScriptById(html, "base-currency") as {
    code?: string;
  } | null;
  const currency = mapEgetatilevleriCurrency(base?.code);
  const periods: MappedVillaPricePeriod[] = [];

  for (const item of raw as EgetatilevleriSeasonalPricing[]) {
    const startKey = normalizeLooseDateKey(String(item.StartDate ?? ""));
    const endKey = normalizeLooseDateKey(String(item.EndDate ?? ""));
    const nightlyPrice = Number(item.PricePerNight);
    if (!startKey || !endKey || !Number.isFinite(nightlyPrice) || nightlyPrice <= 0) {
      continue;
    }
    const startDate = parseDateKey(startKey);
    const endDate = parseDateKey(endKey);
    ...
    const weeklyPrice = Number(item.WeeklyPrice);
    const heatingPrice = Number(item.HeatingPrice);
    const period = buildMappedPeriod({
      sourceId: periods.length + 1,
      startDate,
      endDate,
      nightlyPrice,
      currency,
      weeklyPrice:
        Number.isFinite(weeklyPrice) && weeklyPrice > 0 ? weeklyPrice : null,
      minStayNights: Number(item.MinStayDays),
      cleaningDayCount: Number(item.MinStayDaysForCleaning),
      cleaningFee: Number(item.CleaningPrice),
      cleaningFeeCurrency: currency,
    });
    if (Number.isFinite(heatingPrice) && heatingPrice > 0) {
      period.poolHeatingIndoorFee = Math.round(heatingPrice);
      period.poolHeatingIndoorFeeCurrency = currency;
    }
    periods.push(period);
  }
...
export function parseEgetatilevleriOccupancy(html: string): {
  occupancyByDateKey: Map<string, VillaDayOccupancy>;
  checkInDateKeys: Set<string>;
} {
  const occupancyByDateKey = new Map<string, VillaDayOccupancy>();
  const checkInDateKeys = new Set<string>();
  const raw =
    extractJsonScriptById(html, "booking-availability-ranges") ??
    extractJsonScriptById(html, "booking-blocked-ranges");
  if (!Array.isArray(raw)) {
    return { occupancyByDateKey, checkInDateKeys };
  }

  for (const item of raw as EgetatilevleriBlockedRange[]) {
    const status = String(item.status ?? item.reason ?? "").toLowerCase();
    const booked =
      /blocked|completed|booked|rezerve|dolu/.test(status) ||
      /rezerve|dolu/i.test(String(item.reason ?? ""));
    if (!booked) continue;

    const startKey = normalizeLooseDateKey(String(item.start ?? ""));
    const endKey = normalizeLooseDateKey(String(item.end ?? ""));
    if (!startKey || !endKey) continue;

    let cursor = parseDateKey(startKey);
    const end = parseDateKey(endKey);
    if (compareDates(cursor, end) >= 0) continue;

    checkInDateKeys.add(startKey);
    while (compareDates(cursor, end) < 0) {
      occupancyByDateKey.set(toDateKey(cursor), "BOOKED");
      cursor.setDate(cursor.getDate() + 1);
    }
...
function scrapeEgetatilevleriFromHtml(
  pageUrl: string,
  html: string,
  warnings: string[]
): ScrapedVillaPage | null {
  if (!looksLikeEgetatilevleri(pageUrl, html)) return null;

  const periods = parseEgetatilevleriPeriods(html);
  const { occupancyByDateKey, checkInDateKeys } =
    parseEgetatilevleriOccupancy(html);

  if (periods.length === 0 && occupancyByDateKey.size === 0) {
    warnings.push("egetatilevleri: sezon fiyatı ve takvim okunamadı");
    return null;
  }
  if (periods.length === 0) {
    warnings.push("egetatilevleri: sezon fiyatı bulunamadı");
  }
  if (occupancyByDateKey.size === 0) {
    warnings.push("egetatilevleri: dolu gün bulunamadı");
  }

  return {
    sourceHost: normalizeHost(new URL(pageUrl).hostname),
    strategy: "egetatilevleri",
    pageTitle: extractPageTitle(html),
    periods,
    occupancyByDateKey,
    checkInDateKeys,
...
export async function scrapeExternalVillaPage(
  ...
  const birvillas = scrapeBirvillasFromHtml(normalizedUrl, html, warnings);
  if (birvillas) return finalizeScrapedPage(birvillas, html);

  const plato = scrapePlatoMacrovillaFromHtml(normalizedUrl, html, warnings);
  if (plato) return finalizeScrapedPage(plato, html);

  const egetatilevleri = scrapeEgetatilevleriFromHtml(
    normalizedUrl,
    html,
    warnings
  );
  if (egetatilevleri) return finalizeScrapedPage(egetatilevleri, html);
  ...
  const villajoye = await scrapeVillajoyeFromPage(
    parsed.toString(),
    html,
    warnings
  );
  if (villajoye) return finalizeScrapedPage(villajoye, html);
