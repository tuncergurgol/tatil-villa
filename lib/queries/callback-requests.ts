...
export type PublicRequestGuardEventItem = Awaited<
  ReturnType<typeof getRecentBlockedPublicRequests>
>[number];
...
export async function getRecentBlockedPublicRequests(take = 40) {
  return prisma.publicRequestGuardEvent.findMany({
    where: { blocked: true },
    orderBy: { createdAt: "desc" },
    take,
  });
