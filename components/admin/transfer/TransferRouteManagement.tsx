"use client";

import { useMemo, useState, useTransition } from "react";
import { MapPin, Pencil, Plus, Power, Trash2, X } from "lucide-react";
import {
  createTransferRoute,
  deleteTransferRoute,
  toggleTransferRouteActive,
  updateTransferRoute,
} from "@/app/actions/admin/transfer-routes";
import type { TransferRouteItem } from "@/lib/queries/transfer-routes";

const inputClass =
  "w-full rounded-xl border border-gray-200 bg-gray-50/80 px-4 py-3 text-sm font-medium text-gray-900 outline-none transition focus:border-teal-300 focus:bg-white focus:ring-2 focus:ring-teal-100";

type StatusFilter = "active" | "passive" | "all";

type VehicleOption = {
  id: string;
  code: string;
  name: string;
};

interface Props {
  items: TransferRouteItem[];
  vehicleTypes: VehicleOption[];
  totalCount: number;
  activeCount: number;
}

type PriceDraft = { vehicleTypeId: string; price: string };

function RouteFields({
  item,
  vehicleTypes,
  prices,
  setPrices,
}: {
  item?: TransferRouteItem | null;
  vehicleTypes: VehicleOption[];
  prices: PriceDraft[];
  setPrices: (next: PriceDraft[]) => void;
}) {
  return (
    <div className="space-y-4">
      <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-3">
        <label className="block md:col-span-2">
          <span className="text-xs font-medium text-gray-500">Başlık</span>
          <input
            name="title"
            required
            defaultValue={item?.title ?? ""}
            className={inputClass}
          />
        </label>
        <label className="block">
          <span className="text-xs font-medium text-gray-500">Slug</span>
          <input
            name="slug"
            defaultValue={item?.slug ?? ""}
            placeholder="otomatik"
            className={inputClass}
          />
        </label>
        <label className="block">
          <span className="text-xs font-medium text-gray-500">
            Başlangıç Noktası
          </span>
          <input
            name="startPoint"
            required
            defaultValue={item?.startPoint ?? ""}
            placeholder="Dalaman Havalimanı"
            className={inputClass}
          />
        </label>
        <label className="block">
          <span className="text-xs font-medium text-gray-500">Bitiş Noktası</span>
          <input
            name="endPoint"
            required
            defaultValue={item?.endPoint ?? ""}
            placeholder="Fethiye Merkez"
            className={inputClass}
          />
        </label>
        <label className="block">
          <span className="text-xs font-medium text-gray-500">Mesafe (km)</span>
          <input
            name="distanceKm"
            type="number"
            step="0.1"
            min={0}
            defaultValue={item?.distanceKm ?? ""}
            className={inputClass}
          />
        </label>
        <label className="block">
          <span className="text-xs font-medium text-gray-500">Süre (dk)</span>
          <input
            name="durationMinutes"
            type="number"
            min={0}
            defaultValue={item?.durationMinutes ?? ""}
            className={inputClass}
          />
        </label>
        <label className="block">
          <span className="text-xs font-medium text-gray-500">Öncelik</span>
          <input
            name="priority"
            type="number"
            min={0}
            defaultValue={item?.priority ?? 0}
            className={inputClass}
          />
        </label>
        <label className="block">
          <span className="text-xs font-medium text-gray-500">Etiket</span>
          <input
            name="tag"
            defaultValue={item?.tag ?? ""}
            placeholder="best seller"
            className={inputClass}
          />
        </label>
        <label className="block">
          <span className="text-xs font-medium text-gray-500">SEF URL</span>
          <input
            name="sefUrl"
            defaultValue={item?.sefUrl ?? ""}
            className={inputClass}
          />
        </label>
        <label className="block">
          <span className="text-xs font-medium text-gray-500">
            Havale İndirim (%)
          </span>
          <input
            name="bankTransferDiscountRate"
            type="number"
            step="0.1"
            min={0}
            max={100}
            defaultValue={item?.bankTransferDiscountRate ?? 0}
            className={inputClass}
          />
        </label>
        <label className="block">
          <span className="text-xs font-medium text-gray-500">
            Kredi Kartı İndirim (%)
          </span>
          <input
            name="creditCardDiscountRate"
            type="number"
            step="0.1"
            min={0}
            max={100}
            defaultValue={item?.creditCardDiscountRate ?? 0}
            className={inputClass}
          />
        </label>
        <label className="block">
          <span className="text-xs font-medium text-gray-500">Durum</span>
          <select
            name="isActive"
            defaultValue={item?.isActive === false ? "false" : "true"}
            className={inputClass}
          >
            <option value="true">Aktif</option>
            <option value="false">Pasif</option>
          </select>
        </label>
        <label className="block">
          <span className="text-xs font-medium text-gray-500">Listede Göster</span>
          <select
            name="onList"
            defaultValue={item?.onList === false ? "false" : "true"}
            className={inputClass}
          >
            <option value="true">Evet</option>
            <option value="false">Hayır</option>
          </select>
        </label>
        <label className="block">
          <span className="text-xs font-medium text-gray-500">Online Ödeme</span>
          <select
            name="creditCardPaymentEnabled"
            defaultValue={
              item?.creditCardPaymentEnabled === true ? "true" : "false"
            }
            className={inputClass}
          >
            <option value="false">Kapalı</option>
            <option value="true">Aktif</option>
          </select>
        </label>
        <label className="block md:col-span-2 xl:col-span-3">
          <span className="text-xs font-medium text-gray-500">SEO Başlık</span>
          <input
            name="seoTitle"
            defaultValue={item?.seoTitle ?? ""}
            className={inputClass}
          />
        </label>
        <label className="block md:col-span-2 xl:col-span-3">
          <span className="text-xs font-medium text-gray-500">SEO Açıklama</span>
          <textarea
            name="seoDesc"
            rows={2}
            defaultValue={item?.seoDesc ?? ""}
            className={inputClass}
          />
        </label>
        <label className="block md:col-span-2 xl:col-span-3">
          <span className="text-xs font-medium text-gray-500">
            SEO Anahtar Kelimeler
          </span>
          <input
            name="seoKeywords"
            defaultValue={item?.seoKeywords ?? ""}
            className={inputClass}
          />
        </label>
      </div>

      <div className="rounded-xl border border-gray-200 bg-white p-4">
        <div className="mb-3 flex items-center justify-between gap-3">
          <div>
            <h3 className="text-sm font-semibold text-gray-900">
              Araç Tipi Fiyatları
            </h3>
            <p className="text-xs text-gray-500">
              Sabit rota fiyatı; KM formülüne bakılmaz.
            </p>
          </div>
          <button
            type="button"
            onClick={() =>
              setPrices([
                ...prices,
                {
                  vehicleTypeId: vehicleTypes[0]?.id ?? "",
                  price: "0",
                },
              ])
            }
            disabled={vehicleTypes.length === 0}
            className="inline-flex items-center gap-1 rounded-lg border border-gray-200 px-3 py-1.5 text-xs font-semibold text-gray-700 hover:bg-gray-50 disabled:opacity-50"
          >
            <Plus className="h-3.5 w-3.5" />
            Fiyat Ekle
          </button>
        </div>
        {vehicleTypes.length === 0 ? (
          <p className="text-sm text-amber-700">
            Önce aktif bir araç tipi tanımlayın.
          </p>
        ) : prices.length === 0 ? (
          <p className="text-sm text-gray-500">Henüz fiyat satırı yok.</p>
        ) : (
          <div className="space-y-2">
            {prices.map((row, index) => (
              <div
                key={`${row.vehicleTypeId}-${index}`}
                className="grid gap-2 md:grid-cols-[1fr_160px_40px]"
              >
                <select
                  name="priceVehicleTypeId"
                  value={row.vehicleTypeId}
                  onChange={(e) => {
                    const next = [...prices];
                    next[index] = {
                      ...next[index],
                      vehicleTypeId: e.target.value,
                    };
                    setPrices(next);
                  }}
                  className={inputClass}
                >
                  {vehicleTypes.map((vt) => (
                    <option key={vt.id} value={vt.id}>
                      {vt.name}
                    </option>
                  ))}
                </select>
                <input
                  name="priceAmount"
                  type="number"
                  step="0.01"
                  min={0}
                  value={row.price}
                  onChange={(e) => {
                    const next = [...prices];
                    next[index] = { ...next[index], price: e.target.value };
                    setPrices(next);
                  }}
                  className={inputClass}
                />
                <button
                  type="button"
                  onClick={() =>
                    setPrices(prices.filter((_, i) => i !== index))
                  }
                  className="rounded-xl border border-red-200 text-red-600 hover:bg-red-50"
                >
                  <Trash2 className="mx-auto h-4 w-4" />
                </button>
              </div>
            ))}
          </div>
        )}
      </div>
    </div>
  );
}

export default function TransferRouteManagement({
  items,
  vehicleTypes,
  totalCount,
  activeCount,
}: Props) {
  const [statusFilter, setStatusFilter] = useState<StatusFilter>("active");
  const [showAdd, setShowAdd] = useState(false);
  const [editingId, setEditingId] = useState<string | null>(null);
  const [addPrices, setAddPrices] = useState<PriceDraft[]>([]);
  const [editPrices, setEditPrices] = useState<PriceDraft[]>([]);
  const [error, setError] = useState<string | null>(null);
  const [isPending, startTransition] = useTransition();

  const filtered = useMemo(() => {
    return items.filter((item) => {
      if (statusFilter === "all") return true;
      if (statusFilter === "active") return item.isActive;
      return !item.isActive;
    });
  }, [items, statusFilter]);

  function runCreate(formData: FormData) {
    setError(null);
    startTransition(async () => {
      const result = await createTransferRoute({}, formData);
      if (result.error) {
        setError(result.error);
        return;
      }
      setShowAdd(false);
      setAddPrices([]);
    });
  }

  function runUpdate(formData: FormData) {
    setError(null);
    startTransition(async () => {
      const result = await updateTransferRoute({}, formData);
      if (result.error) {
        setError(result.error);
        return;
      }
      setEditingId(null);
      setEditPrices([]);
    });
  }

  function runDelete(id: string, title: string) {
    if (!window.confirm(`"${title}" rotası silinsin mi?`)) return;
    setError(null);
    startTransition(async () => {
      const result = await deleteTransferRoute(id);
      if (result.error) setError(result.error);
    });
  }

  function runToggle(id: string) {
    setError(null);
    startTransition(async () => {
      const result = await toggleTransferRouteActive(id);
      if (result.error) setError(result.error);
    });
  }

  return (
    <div className="space-y-6">
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          <p className="text-xs font-semibold tracking-[0.2em] text-gray-400 uppercase">
            Transfer
          </p>
          <h1 className="mt-1 text-3xl font-bold text-gray-900">Rotalar</h1>
          <p className="mt-2 max-w-2xl text-sm text-gray-500">
            Başlangıç–bitiş, mesafe, süre ve araç tipi sabit fiyatları.
          </p>
        </div>
        <button
          type="button"
          onClick={() => {
            setShowAdd(true);
            setEditingId(null);
            setAddPrices([]);
          }}
          className="inline-flex items-center gap-2 rounded-xl bg-violet-600 px-4 py-2.5 text-sm font-semibold text-white hover:bg-violet-700"
        >
          <Plus className="h-4 w-4" />
          Yeni Rota
        </button>
      </div>

      <div className="grid gap-4 sm:grid-cols-3">
        <div className="rounded-2xl border border-gray-200 bg-white p-5">
          <p className="text-sm text-gray-500">Toplam</p>
          <p className="text-2xl font-bold text-gray-900">{totalCount}</p>
        </div>
        <div className="rounded-2xl border border-gray-200 bg-white p-5">
          <p className="text-sm text-gray-500">Aktif</p>
          <p className="text-2xl font-bold text-gray-900">{activeCount}</p>
        </div>
        <div className="rounded-2xl border border-gray-200 bg-white p-5">
          <p className="text-sm text-gray-500">Pasif</p>
          <p className="text-2xl font-bold text-gray-900">
            {totalCount - activeCount}
          </p>
        </div>
      </div>

      {error ? (
        <div className="rounded-xl border border-red-200 bg-red-50 px-4 py-3 text-sm text-red-700">
          {error}
        </div>
      ) : null}

      {showAdd ? (
        <form
          action={runCreate}
          className="space-y-4 rounded-2xl border border-violet-200 bg-violet-50/40 p-5"
        >
          <div className="flex items-center justify-between gap-3">
            <h2 className="text-lg font-semibold text-gray-900">Yeni Rota</h2>
            <button type="button" onClick={() => setShowAdd(false)}>
              <X className="h-5 w-5 text-gray-500" />
            </button>
          </div>
          <RouteFields
            vehicleTypes={vehicleTypes}
            prices={addPrices}
            setPrices={setAddPrices}
          />
          <div className="flex gap-2">
            <button
              type="submit"
              disabled={isPending}
              className="rounded-xl bg-violet-600 px-4 py-2.5 text-sm font-semibold text-white disabled:opacity-60"
            >
              Kaydet
            </button>
            <button
              type="button"
              onClick={() => setShowAdd(false)}
              className="rounded-xl border border-gray-200 bg-white px-4 py-2.5 text-sm font-semibold text-gray-700"
            >
              İptal
            </button>
          </div>
        </form>
      ) : null}

      <div className="flex rounded-xl border border-gray-200 p-1 w-fit">
        {(
          [
            ["active", "Aktif"],
            ["passive", "Pasif"],
            ["all", "Tümü"],
          ] as const
        ).map(([value, label]) => (
          <button
            key={value}
            type="button"
            onClick={() => setStatusFilter(value)}
            className={`rounded-lg px-3 py-1.5 text-sm font-medium transition ${
              statusFilter === value
                ? "bg-teal-600 text-white"
                : "text-gray-600 hover:bg-gray-50"
            }`}
          >
            {label}
          </button>
        ))}
      </div>

      <div className="overflow-hidden rounded-xl border border-gray-200 bg-white">
        <table className="w-full text-left text-sm">
          <thead className="border-b border-gray-200 bg-gray-50 text-xs font-semibold uppercase tracking-wide text-gray-500">
            <tr>
              <th className="px-4 py-3">Rota</th>
              <th className="px-4 py-3">Mesafe / Süre</th>
              <th className="px-4 py-3">Araç Fiyatları</th>
              <th className="px-4 py-3">Durum</th>
              <th className="w-56 px-4 py-3 text-right">İşlemler</th>
            </tr>
          </thead>
          <tbody>
            {filtered.length === 0 ? (
              <tr>
                <td colSpan={5} className="px-4 py-12 text-center text-gray-500">
                  Kayıt bulunamadı.
                </td>
              </tr>
            ) : (
              filtered.map((item) =>
                editingId === item.id ? (
                  <tr key={item.id} className="border-t border-gray-100 bg-violet-50/30">
                    <td colSpan={5} className="px-4 py-4">
                      <form action={runUpdate} className="space-y-4">
                        <input type="hidden" name="id" value={item.id} />
                        <RouteFields
                          item={item}
                          vehicleTypes={vehicleTypes}
                          prices={editPrices}
                          setPrices={setEditPrices}
                        />
                        <div className="flex gap-2">
                          <button
                            type="submit"
                            disabled={isPending}
                            className="rounded-lg bg-violet-600 px-4 py-2 text-sm font-semibold text-white"
                          >
                            Kaydet
                          </button>
                          <button
                            type="button"
                            onClick={() => setEditingId(null)}
                            className="rounded-lg border border-gray-200 px-4 py-2 text-sm font-semibold text-gray-700"
                          >
                            İptal
                          </button>
                        </div>
                      </form>
                    </td>
                  </tr>
                ) : (
                  <tr key={item.id} className="border-t border-gray-100">
                    <td className="px-4 py-3">
                      <div className="flex items-start gap-3">
                        <div className="rounded-xl bg-teal-50 p-2 text-teal-600">
                          <MapPin className="h-4 w-4" />
                        </div>
                        <div>
                          <p className="font-semibold text-gray-900">
                            {item.title}
                          </p>
                          <p className="text-xs text-gray-500">
                            {item.startPoint} → {item.endPoint}
                          </p>
                        </div>
                      </div>
                    </td>
                    <td className="px-4 py-3 text-gray-700">
                      {item.distanceKm != null ? `${item.distanceKm} km` : "—"}
                      {" · "}
                      {item.durationMinutes != null
                        ? `${item.durationMinutes} dk`
                        : "—"}
                    </td>
                    <td className="px-4 py-3 text-gray-700">
                      {item.vehiclePrices.length === 0 ? (
                        <span className="text-gray-400">—</span>
                      ) : (
                        <div className="space-y-0.5 text-xs">
                          {item.vehiclePrices.slice(0, 3).map((p) => (
                            <div key={p.id}>
                              {p.vehicleTypeName}: {p.price}
                            </div>
                          ))}
                          {item.vehiclePrices.length > 3 ? (
                            <div>+{item.vehiclePrices.length - 3} daha</div>
                          ) : null}
                        </div>
                      )}
                    </td>
                    <td className="px-4 py-3">
                      <span
                        className={`inline-flex rounded-full px-2.5 py-1 text-xs font-semibold ${
                          item.isActive
                            ? "bg-emerald-50 text-emerald-700"
                            : "bg-gray-100 text-gray-600"
                        }`}
                      >
                        {item.isActive ? "Aktif" : "Pasif"}
                      </span>
                    </td>
                    <td className="px-4 py-3">
                      <div className="flex justify-end gap-2">
                        <button
                          type="button"
                          onClick={() => {
                            setEditingId(item.id);
                            setShowAdd(false);
                            setEditPrices(
                              item.vehiclePrices.map((p) => ({
                                vehicleTypeId: p.vehicleTypeId,
                                price: String(p.price),
                              }))
                            );
                          }}
                          className="inline-flex items-center gap-1 rounded-lg border border-gray-200 px-2.5 py-1.5 text-xs font-semibold text-gray-700 hover:bg-gray-50"
                        >
                          <Pencil className="h-3.5 w-3.5" />
                          Düzenle
                        </button>
                        <button
                          type="button"
                          onClick={() => runToggle(item.id)}
                          disabled={isPending}
                          className="inline-flex items-center gap-1 rounded-lg border border-gray-200 px-2.5 py-1.5 text-xs font-semibold text-gray-700 hover:bg-gray-50"
                        >
                          <Power className="h-3.5 w-3.5" />
                          {item.isActive ? "Pasif" : "Aktif"}
                        </button>
                        <button
                          type="button"
                          onClick={() => runDelete(item.id, item.title)}
                          disabled={isPending}
                          className="inline-flex items-center gap-1 rounded-lg border border-red-200 px-2.5 py-1.5 text-xs font-semibold text-red-700 hover:bg-red-50"
                        >
                          <Trash2 className="h-3.5 w-3.5" />
                          Sil
                        </button>
                      </div>
                    </td>
                  </tr>
                )
              )
            )}
          </tbody>
        </table>
      </div>
    </div>
  );
}
