...
export default function BlogInspirationSlider({
  ...
  const items = useMemo(
    () => posts.filter((p) => p.title.trim().length > 0),
    [posts]
  );
  ...
  if (items.length === 0) {
    return (
      <div className="rounded-[2rem] border border-dashed border-sky-200 bg-white/70 p-10 text-center text-slate-500">
        Henüz yazı yok.
      </div>
    );
  }
  ...
  return (
    <section className="overflow-hidden rounded-[2.25rem] border border-sky-100/80 bg-[linear-gradient(135deg,#eef9ff_0%,#fff7fb_48%,#ffffff_100%)] p-5 shadow-[0_16px_50px_rgba(56,189,248,0.12)] sm:p-8">
