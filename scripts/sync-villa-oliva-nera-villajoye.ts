...
async function main() {
  ...
  const candidates = await prisma.villa.findMany({
    where: {
      OR: [
        { name: { contains: "Oliva Nera", mode: "insensitive" } },
        { slug: { contains: "oliva-nera", mode: "insensitive" } },
      ],
    },
    select: {
      id: true,
  ...
  const villa =
    candidates.find((row) => row.slug === "villa-oliva-nera-dalyan") ??
    candidates.find((row) => row.name.toLowerCase().includes("oliva nera"));

  if (!villa) {
    throw new Error("Villa Oliva Nera bulunamadı — --list ile adayları kontrol edin");
  }
