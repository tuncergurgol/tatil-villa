/**
 * Villa Nisan (820) — AkdenizdeVilla Link 1
 *   npx tsx scripts/sync-villa-nisan-akdenizdevilla.ts --list
 *   npx tsx scripts/sync-villa-nisan-akdenizdevilla.ts --dry-run
 *   npx tsx scripts/sync-villa-nisan-akdenizdevilla.ts
 */
...
const URL =
  "https://www.akdenizdevilla.com/tr/kiralik-villalar/villa-nisan";
...
async function main() {
  ...
  const candidates = await prisma.villa.findMany({
    where: {
      OR: [
        { name: { equals: "Villa Nisan", mode: "insensitive" } },
        { slug: { equals: "villa-nisan", mode: "insensitive" } },
        { villaId: 820 },
      ],
    },
    select: {
  ...
  const villa =
    candidates.find((row) => row.villaId === 820) ??
    candidates.find((row) => row.slug.toLowerCase() === "villa-nisan") ??
    candidates.find((row) => row.name.toLowerCase() === "villa nisan");

  if (!villa) {
    throw new Error("Villa Nisan bulunamadı — --list ile adayları kontrol edin");
  }
