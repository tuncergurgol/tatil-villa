/**
 * Villa periyotlarında temizlik gün sayısını toplu günceller.
 *
 * Kullanım:
 *   npx tsx scripts/update-villa-period-cleaning-day.ts --dry-run --names-file list.txt
 *   npx tsx scripts/update-villa-period-cleaning-day.ts --names-file list.txt --days 7
 */
...
function parseArgs(): Args {
  ...
  if (daysIdx >= 0) {
    ...
    if (!raw) throw new Error("--days için değer gerekli");
...
async function main() {
  ...
  console.log("Hedef temizlik gün sayısı:", args.days);
  ...
  const periodResult = await prisma.villaPricePeriod.updateMany({
    ...
    data: { cleaningDayCount: args.days },
  });
