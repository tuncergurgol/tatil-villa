/**
 * Match our Villa.documentNo against villavillam.com.tr (sitemap + __NEXT_DATA__.gavelBelgeNo).
 *
 *   npx tsx scripts/match-villavillam-by-document.ts
 *   npx tsx scripts/match-villavillam-by-document.ts --limit=50
 *   npx tsx scripts/match-villavillam-by-document.ts --resume
 *   npx tsx scripts/match-villavillam-by-document.ts --concurrency=3 --delay-ms=400
 *
 * Outputs:
 *   scripts/villavillam-match-report.json
 *   scripts/villavillam-match-report.csv
 *   scripts/villavillam-catalog-cache.json (resume cache)
 */
import { createWriteStream, existsSync, readFileSync, writeFileSync } from "fs";
import { prisma } from "../lib/db";

const BASE = "https://www.villavillam.com.tr";
const CACHE_PATH = "scripts/villavillam-catalog-cache.json";
const REPORT_JSON = "scripts/villavillam-match-report.json";
const REPORT_CSV = "scripts/villavillam-match-report.csv";
...
type MatchRow = {
  ...
  ourSlug: string;
  ourActive: boolean;
  villavillamUrl: string;
  villavillamSlug: string;
  villavillamName: string;
  villavillamBelgeNo: string;
...
function parseCatalogFromHtml(url: string, html: string): CatalogEntry {
  ...
  const result = next?.props?.pageProps?.data?.result;
  const routingType = next?.props?.pageProps?.data?.RoutingTypeId;

  if (!result || routingType !== "ProductDetail") {
    return {
      url,
      slug,
      name: "",
      ...
      entityId: "",
      fetchStatus: "skip",
      error: `not ProductDetail (${routingType ?? "no-next"})`,
    };
  }
  ...
  return {
    ...
    name: String(result.baslik || result.title || "").trim(),
    belgeNo: String(result.gavelBelgeNo || "").trim(),
...
async function loadSitemapVillaUrls(): Promise<string[]> {
  ...
  const villas = locs.filter((u) => {
    try {
      ...
      return /^\/villa-[a-z0-9-]+$/i.test(path) && path !== "/villa-secenekleri";
    } catch {
      return false;
    }
  });
  return [...new Set(villas.map((u) => u.replace(/\/$/, "")))];
}
...
function writeCsv(rows: MatchRow[]) {
  ...
  for (const row of rows) {
    stream.write(headers.map((h) => csvEscape(row[h])).join(",") + "\n");
  }
  stream.end();
}
  ...
  for (const villa of ours) {
    ...
    if (belgeKey) {
      ...
      if (hits.length === 1) {
        ...
        rows.push({
          ...
          villavillamUrl: hit.url,
          villavillamSlug: hit.slug,
          villavillamName: hit.name,
          villavillamBelgeNo: hit.belgeNo,
          matchConfidence: "exact",
          matchNote: "documentNo == gavelBelgeNo",
        });
        continue;
      }
      if (hits.length > 1) {
        ...
        rows.push({
          ...
          ourSlug: villa.slug,
          ourActive: villa.active,
          villavillamUrl: hits.map((h) => h.url).join(" | "),
          villavillamSlug: hits.map((h) => h.slug).join(" | "),
          villavillamName: hits.map((h) => h.name).join(" | "),
          villavillamBelgeNo: hits.map((h) => h.belgeNo).join(" | "),
          matchConfidence: "ambiguous",
          matchNote: `${hits.length} villavillam pages share this belgeNo`,
        });
        continue;
      }
    }
    ...
    if (slugHit && !usedUrls.has(slugHit.url)) {
      ...
      if (belgeKey && theirs && belgeKey !== theirs) {
        ...
        rows.push({
          ...
          ourSlug: villa.slug,
          ourActive: villa.active,
          villavillamUrl: slugHit.url,
          villavillamSlug: slugHit.slug,
          villavillamName: slugHit.name,
          villavillamBelgeNo: slugHit.belgeNo,
          matchConfidence: "ambiguous",
    ...
    if (bestOk && bestOk.score >= 0.72) {
      ...
      rows.push({
        ...
        ourSlug: villa.slug,
        ourActive: villa.active,
        villavillamUrl: bestOk.entry.url,
        villavillamSlug: bestOk.entry.slug,
        villavillamName: bestOk.entry.name,
        villavillamBelgeNo: bestOk.entry.belgeNo,
        matchConfidence: "fuzzy",
        matchNote: belgeKey
    ...
    if (bestConflict && bestConflict.score >= 0.85) {
      ...
      rows.push({
        ...
        ourSlug: villa.slug,
        ourActive: villa.active,
        villavillamUrl: bestConflict.entry.url,
        villavillamSlug: bestConflict.entry.slug,
        villavillamName: bestConflict.entry.name,
        villavillamBelgeNo: bestConflict.entry.belgeNo,
        matchConfidence: "ambiguous",
      ...
      matchNote: belgeKey
        ? bestConflict
          ...
          : bestOk
            ...
            : "belge not found on villavillam"
...
async function main() {
  ...
  const summary = {
    ...
    villavillamCatalog: catalog.length,
    villavillamWithBelge: catalog.filter((c) => c.belgeNo.trim()).length,
    matches: {
      exact,
      fuzzy,
      ambiguous,
