/**
 * Excel eşleştirme sheet'inden villavillam.com.tr URL'lerini
 * Villa.externalSyncUrl1 (Link 1) alanına yazar ve hemen sync eder.
 *
 * Sadece Link 1 boş villalar işlenir. exact/fuzzy + URL dolu satırlar.
 *
 *   npx tsx scripts/apply-villavillam-external-sync.ts
 *   npx tsx scripts/apply-villavillam-external-sync.ts --xlsx=C:/path/to.xlsx
 *   npx tsx scripts/apply-villavillam-external-sync.ts --dry-run
 *   npx tsx scripts/apply-villavillam-external-sync.ts --delay-ms=800
 *   npx tsx scripts/apply-villavillam-external-sync.ts --limit=10
 */
...
function rowUrl(row: SheetRow): string {
  return String(row.villavillamUrl || row.externalUrl || "").trim();
}
...
async function main() {
  ...
  console.log(
    `Eşleşen satır (exact/fuzzy + URL): ${toProcess.length}; skip (none/ambiguous/boş): ${skippedNoMatch}`
  );
  ...
  console.log(`Excel eşleşme (exact/fuzzy+URL): ${toProcess.length}`);
  ...
  console.log(
    `Skip diğer: ${skipped} (+ confidence/boş: ${skippedNoMatch})`
  );
  if (errors.length) {
