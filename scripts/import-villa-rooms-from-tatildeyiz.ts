...
const REPORT_PATH = path.join(
  process.cwd(),
  "scripts",
  "import-villa-rooms-report.json"
);
...
async function loadPreviousReport(
  ...
  try {
    ...
    const report = JSON.parse(raw) as { results?: ImportVillaRoomsResult[] };
...
async function main() {
  ...
  const results: ImportVillaRoomsResult[] = [];
  ...
  console.log(
    `Tatildeyiz oda import başlıyor (${slugs.length} villa, dryRun=${options.dryRun}, force=${options.force})`
  );
  ...
  for (let index = 0; index < slugs.length; index += 1) {
    ...
    try {
      const result = await applyTatildeyizRoomsToVilla(prisma, slug, {
  ...
  const report = {
    ...
    summary: {
      ...
      totalUpdated: mergedResults.reduce(
  ...
  console.log(
    `Özet: ${report.summary.success} başarılı, ${report.summary.skipped} atlandı, ${report.summary.error} hata, ${report.summary.totalUpdated} oda güncellendi`
  );
}
