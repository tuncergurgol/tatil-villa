...
import {
  ...
  RESERVATION_DOCUMENT_SENT_MANAGEMENT_MESSAGE_NAME,
...
async function upsertTemplate() {
  const existing = await prisma.agencyMessageTemplate.findFirst({
    where: {
      rowNo: AGENCY_MESSAGE_TEMPLATE_ROW_20_5,
      active: true,
    },
  ...
  const data = {
    rowNo: AGENCY_MESSAGE_TEMPLATE_ROW_20_5,
    name: RESERVATION_DOCUMENT_SENT_MANAGEMENT_MESSAGE_NAME,
    recipient: RECIPIENT,
  ...
  if (existing) {
    ...
    console.log(
      `${formatAgencyMessageRowNo(AGENCY_MESSAGE_TEMPLATE_ROW_20_5)} mesaj şablonu güncellendi.`
    );
    return;
  }
  ...
  console.log(
    `${formatAgencyMessageRowNo(AGENCY_MESSAGE_TEMPLATE_ROW_20_5)} mesaj şablonu eklendi.`
  );
}
