/**
 * Match our Villa.documentNo against kiralikvilladatatil.com
 * (sitemap /kiralik-villa/* + HTML "BELGE NO" near Kültür ve Turizm).
 *
 *   npx tsx scripts/match-kiralikvilladatatil-by-document.ts
 *   npx tsx scripts/match-kiralikvilladatatil-by-document.ts --limit=50
 *   npx tsx scripts/match-kiralikvilladatatil-by-document.ts --resume
 *   npx tsx scripts/match-kiralikvilladatatil-by-document.ts --concurrency=3 --delay-ms=400
 *   npx tsx scripts/match-kiralikvilladatatil-by-document.ts --match-only
 *
 * Outputs:
 *   scripts/kiralikvilladatatil-match-report.json
 *   scripts/kiralikvilladatatil-match-report.csv
 *   scripts/kiralikvilladatatil-match-report.xlsx
 *   scripts/kiralikvilladatatil-catalog-cache.json
 *   Desktop: kiralikvilladatatil-eslestirme.xlsx
 */
...
const BASE = "https://www.kiralikvilladatatil.com";
const CACHE_PATH = "scripts/kiralikvilladatatil-catalog-cache.json";
const REPORT_JSON = "scripts/kiralikvilladatatil-match-report.json";
const REPORT_CSV = "scripts/kiralikvilladatatil-match-report.csv";
const REPORT_XLSX = "scripts/kiralikvilladatatil-match-report.xlsx";
...
/** Tourism belge near Kültür ve Turizm; ignore TURSAB footer numbers. */
function extractBelgeNo(html: string): string {
  const nearKultur = html.match(
    /Turizm\s*Bakanl[\s\S]{0,500}?BELGE\s*NO\s*:\s*([^<\s]+)/i
  );
  if (nearKultur?.[1]) return nearKultur[1].trim();

  const dashed = html.match(/BELGE\s*NO\s*:\s*(\d{2}[-./]\d[\d./-]*)/i);
...
function parseCatalogFromHtml(url: string, html: string): CatalogEntry {
  ...
  // Soft skip: non-product / soft / blocking pages rarely have tourism belge block
  if (!/kiralik-villa\//i.test(path)) {
    return {
      url,
      slug,
      name: "",
...
async function loadSitemapVillaUrls(): Promise<string[]> {
  ...
  const villas = locs.filter((u) => {
    try {
      ...
      return /^\/kiralik-villa\/[a-z0-9-]+$/i.test(path);
    } catch {
      return false;
    }
  });
    ...
    if (belgeKey) {
      ...
      if (hits.length > 1) {
        ...
        rows.push({
          ...
          matchNote: `${hits.length} kiralikvilladatatil pages share this belgeNo`,
        });
        continue;
      }
    }
      ...
      matchNote: belgeKey
        ? bestConflict
          ...
          : bestOk
            ...
            : "belge not found on kiralikvilladatatil"
